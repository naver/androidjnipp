macro(WRAP_SOURCELIST)
    foreach (_file ${ARGN})
        ...
        if (NOT _file MATCHES "${CMAKE_CURRENT_BINARY_DIR}/GeneratedInterfaces")
            string(REGEX REPLACE "/" "\\\\\\\\" _sourcegroup "${_path}")
            source_group("${_sourcegroup}" FILES ${_file})
        endif ()
    endforeach ()

    source_group("GeneratedInterfaces" REGULAR_EXPRESSION "${CMAKE_CURRENT_BINARY_DIR}/GeneratedInterfaces")
endmacro()
...
macro(COPY_LIBRARY_HEADERS _target _list _destination)
    if (CMAKE_HOST_WIN32)
        file(APPEND "${${_target}_POST_BUILD_COMMAND}" "@mkdir \"${LIBRARY_PRODUCT_DIR}/${_destination}\" >nul 2>nul\n")
        foreach (_file ${_list})
            get_filename_component(_absolute "${_file}" ABSOLUTE)
            file(APPEND "${${_target}_POST_BUILD_COMMAND}" "@xcopy /y /d /f \"${_absolute}\" \"${LIBRARY_PRODUCT_DIR}/${_destination}\" >nul 2>nul\n")
        endforeach ()
    else ()
        add_custom_command(TARGET ${_target} POST_BUILD COMMAND echo Copying ${_target} library headers... VERBATIM)
        foreach (_file ${_list})
            get_filename_component(_absolute "${_file}" ABSOLUTE)
            add_custom_command(TARGET ${_target} POST_BUILD COMMAND mkdir -p ${LIBRARY_PRODUCT_DIR}/${_destination} && cp -u ${_absolute} ${LIBRARY_PRODUCT_DIR}/${_destination} VERBATIM)
        endforeach ()
    endif ()
endmacro()
...
macro(COPY_LIBRARY_HEADERS_DIRECTORY _target _source _destination)
    ...
    if (CMAKE_HOST_WIN32)
        file(APPEND "${${_target}_POST_BUILD_COMMAND}" "@robocopy /s /e \"${_absolute}/\" \"${LIBRARY_PRODUCT_DIR}/${_destination}/\" \"*.h\" \"*.hpp\" >nul 2>nul\n")
        file(APPEND "${${_target}_POST_BUILD_COMMAND}" "IF %ERRORLEVEL% LEQ 3 set ERRORLEVEL=0\n")
    else ()
        add_custom_command(TARGET ${_target} POST_BUILD COMMAND echo Copying ${_target} library headers... VERBATIM)
        add_custom_command(TARGET ${_target} POST_BUILD COMMAND mkdir -p ${LIBRARY_PRODUCT_DIR}/${_destination} && rsync -rpuL --include='*.h' --include='*.hpp' --exclude='*.*' ${_absolute}/* ${LIBRARY_PRODUCT_DIR}/${_destination})
    endif ()
endmacro()
